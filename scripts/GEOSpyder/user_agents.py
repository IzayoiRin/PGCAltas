...
POOL = {'pc': _PC_EQUIPMENT_POOL, 'move': _MOVE_EQUIPMENT_POOL}


def randagent(ends=None):
    import random
    if ends is None:
        pool = _PC_EQUIPMENT_POOL.extend(_MOVE_EQUIPMENT_POOL)
    else:
        pool = POOL.get(ends)
    assert pool, "param 'ends' must be pc or move"
