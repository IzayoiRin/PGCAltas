...
from pgc_torch.conf import CONV_DIM, CONV_BACTH_NOR
from pgc_torch.errors import ConfigError, ObjectTypeError
...
def set_conv_dim():
    ...
    if CONV_DIM in mapping.keys():
        return mapping[CONV_DIM]
    raise ConfigError("set legal conv dim")


Conv, BatchNorm, MaxPool, AvgPool = set_conv_dim()
...
class InceptionBaseRoute(nn.Module):
    ...
    def assemble(self, in_chanel):
        ...
        for i in range(n):
            ...
            if func_n not in self.FMapping.keys():
                raise ConfigError("set legal function name")
    ...
    def forward(self, X):
        if not self._ginit:
            raise ConfigError('assemble computational graph first')
        return self.graph(X)
...
class Inception(nn.Module):
    ...
    def assemble(self, in_chanel):
        ...
        if len(self.Routes) != len(self.chanelArr):
            raise ConfigError("Chanels' length(%s) equal to Routes'(%s)"
